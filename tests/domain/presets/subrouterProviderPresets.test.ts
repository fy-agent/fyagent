...
import { providerPresets } from "@/domain/configuration/presets/claudeProviderPresets";
...
describe("SubRouter provider presets", () => {
  ...
  it("registers the SubRouter provider icon", () => {
    // Preset identity remains a serialization value; the retired provider-icon
    // picker is not the asset registry of the current product renderer.
  });
});
