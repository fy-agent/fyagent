...
describe("Codex TOML utils", () => {
  it("preserves literal replacement metacharacters in bearer tokens", () => {
    const source = 'experimental_bearer_token = "old" # keep\n';
    const updated = updateCodexExperimentalBearerToken(
      source,
      "literal-$&-$1-$$-token",
    );
    expect(updated).toContain(
      'experimental_bearer_token = "literal-$&-$1-$$-token" # keep',
    );
  });
