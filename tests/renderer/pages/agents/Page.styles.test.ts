...
const catalogCss = readFileSync(
  path.resolve(repositoryRoot, "src", "shared", "ui", "catalog", "catalog.css"),
  "utf8",
);
const splitCss = readFileSync(
  path.resolve(repositoryRoot, "src", "shared", "ui", "split", "split.css"),
  "utf8",
);
const featuresCss = readFileSync(
  path.resolve(repositoryRoot, "src", "app", "styles", "features.css"),
  "utf8",
);
const pageCss = ["agents", "models"]
  .map((page) =>
    readFileSync(
      path.resolve(repositoryRoot, "src", "pages", page, "Page.css"),
      "utf8",
    ),
  )
  .join("\n");
...
describe("shared catalog presentation styles", () => {
