  ...
  fs.readFileSync(
    path.resolve(
      [
        ...
        ? "src/pages/models/apply"
        : "src/shared/features/change-plans-ui",
      name,
    ),
    "utf8",
  );
...
describe("Change Plan workflow ownership", () => {
  it("shares save orchestration and delegates all automatic job reads to Query", () => {
    ...
    const models = fs.readFileSync("src/pages/models/Page.tsx", "utf8");
    expect(models).not.toContain("<ChangePlanWorkspace");
    expect(
      fs.readFileSync("src/pages/auth/CodexRequestSource.tsx", "utf8"),
    ).toContain("<ChangePlanWorkspace");
    expect(
      fs.readFileSync("src/pages/agents/AgentConfiguration.tsx", "utf8"),
