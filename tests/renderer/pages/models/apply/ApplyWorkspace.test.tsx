...
import { createApplyViewModel } from "@/shared/features/change-plans-ui/view-model";
import type {
  ChangeJobSnapshot,
  ChangePlan,
...
describe("ApplyWorkspace", () => {
  ...
  it("keeps prohibited prototype controls and data sources out of product code", () => {
    const sourceDir = resolve("src/shared/features/change-plans-ui");
    const sources =
      ["ApplyWorkspace.tsx", "view-model.ts"]
        .map((name) => readFileSync(resolve(sourceDir, name), "utf8"))
        .join("\n") +
      readFileSync(resolve("src/pages/models/apply/index.ts"), "utf8");
