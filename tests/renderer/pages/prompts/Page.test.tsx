    ...
    [
      {
        path: "/prompts",
        element: (
          <PromptsRouteFixture showNavigationControl={showNavigationControl} />
        ),
      },
...
describe("PromptsPage native business management", () => {
  it("loads Claude by default and keeps all seven applications independent", async () => {
    ...
    expect(
      Array.from(document.querySelectorAll('[data-testid^="prompt-app-"]')).map(
  ...
  it("blocks route navigation with the same dirty-state confirmation", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "打开记忆测试目标" }));
