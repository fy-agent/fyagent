...
async function confirmDialog(
  ...
  // Exit revokes accessibility before Radix removes its focus-trapped portal.
  // Do not start typing into the destination during that remaining lifetime.
...
describe("MemoryPage native business management", () => {
  ...
  it("copies the current long-term and daily memory drafts instead of their paths", async () => {
    ...
    await waitFor(() =>
      expect(longTermEditor).toHaveValue("long-term baseline"),
    );
    await user.type(longTermEditor, " + local draft");
    await user.click(screen.getByRole("button", { name: "复制记忆内容" }));
    ...
    await waitFor(() => expect(dailyEditor).toHaveValue("daily baseline"));
    expect(dailyEditor).toBeEnabled();
    await user.type(dailyEditor, " + local draft");
    await user.click(screen.getByRole("button", { name: "复制每日记忆内容" }));
  ...
  it("cancels dirty route navigation with the shared dialog", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "打开其他测试页面" }));
    await cancelDialog(user, "放弃未保存的更改？");
