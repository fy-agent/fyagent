...
async function installAgentDirectoryOverrides(page: Page): Promise<void> {
...
async function installFixture(
  ...
  await installAgentDirectoryOverrides(page);
}

test("Agent directory shows the full catalog, auto-scans, and reuses existing Skill and MCP assignments", async ({
  page,
}) => {
...
test("Agent directory restores deep links and keeps model and prompt capability boundaries honest", async ({
  page,
}) => {
  await installFixture(page);
  const health = monitorPageHealth(page);
  await openRendererPage(page, "/agents?target=qoderwork&section=models");
  ...
  await openRendererPage(page, "/agents?target=trae-work&section=models");
...
test("Agent Auth keeps Claude local verification and routes managed consumers centrally", async ({
  ...
  await openRendererPage(page, "/agents?target=claude-code&section=models");
  ...
  await openRendererPage(page, "/agents?target=grokbuild&section=models");
  ...
  await openRendererPage(page, "/agents?target=opencode&section=models");
