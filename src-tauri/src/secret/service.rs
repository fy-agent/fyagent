...
impl SecretServiceLocal {
    ...
    pub(crate) fn list_secret_summaries(
        ...
        let payload = self.store.load()?.payload;
    ...
    pub(crate) fn discard_secret_candidate(
        ...
        write_journal(self.store.root(), &journal).map_err(|_| SecretInternalError::input_invalid())?;
        ...
        write_journal(self.store.root(), &journal).map_err(|_| SecretInternalError::input_invalid())?;
        self.set_pending_disposition(candidate_id, Some(TerminalDisposition::Discarded))?;
        ...
        write_journal(self.store.root(), &journal).map_err(|_| SecretInternalError::input_invalid())?;
        self.finalize_discarded_candidate(candidate_id)?;
    ...
    fn finalize_discarded_candidate(&self, candidate_id: &str) -> Result<(), SecretInternalError> {
        let mut payload = self.store.load()?.payload;
        let candidate = payload
            .candidates
            .iter()
