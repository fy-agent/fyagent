...
/// OpenCode has a dedicated revisioned writer; generic quick setup stays closed.
#[tauri::command(rename_all = "camelCase")]
pub async fn bind_opencode_managed_proxy(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let state = app_handle
            .try_state::<AppState>()
            .ok_or(BindManagedProxyError::ApplyFailedRolledBack)?;
        ProviderService::bind_opencode_managed_proxy(state.inner(), auth.as_ref(), request)
    })
    .await
    .map_err(|_| BindManagedProxyError::RollbackPartialStateUnknown)?
}
