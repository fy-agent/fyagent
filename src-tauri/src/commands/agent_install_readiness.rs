...
use crate::{
    agent_install::{
        ...
        AgentActionResult, AgentInstallPreflightDto, AgentInstallReadinessDto,
        AgentInstallationInventoryDto, AgentSurface, StartAgentActionRequest,
    },
...
#[tauri::command(rename_all = "camelCase")]
pub async fn get_agent_install_preflight(
    request: StartAgentActionRequest,
    state: State<'_, AppState>,
) -> Result<AgentInstallPreflightDto, AgentActionErrorDto> {
    crate::agent_install::preflight_for(request, &state)
        .await
