...
pub async fn get_opencode_models() -> Result<Vec<OpenCodeModelRef>, String> {
    tokio::task::spawn_blocking(|| {
        ...
        let output = crate::services::tooling::run_detected_tool_command_with_timeout(
            "opencode",
            &["models"],
        ...
        if !output.status.success() {
            let stderr = crate::services::tooling::decode_command_output(&output.stderr);
            let stdout = crate::services::tooling::decode_command_output(&output.stdout);
        ...
        Ok(parse_opencode_models(
            &crate::services::tooling::decode_command_output(&output.stdout),
        ))
    })
    .await
