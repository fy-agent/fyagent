...
- [ ] 用户确认最终规划后，按顺序 task.py start 对应子任务。
- [ ] 完成导航性能子任务；记录生产基线/改后数据，SPEC、工作提交、归档。
- [ ] 完成材质和容器响应子任务；图像与对比度矩阵、SPEC、工作提交、归档。
- [ ] 完成动效子任务；有源/无源、开关竞争、可访问性和帧性能，SPEC、提交、归档。
- [ ] 启动父任务整合检查；审查完整diff及命名/引用映射，复跑性能与组合态UI。
- [ ] 运行 mise run typecheck:v2、lint:v2、test:v2、test:v2:browser、build:renderer 及项目完整 check:prearchive；使用runner规定的精确任务排除，不新增豁免。
