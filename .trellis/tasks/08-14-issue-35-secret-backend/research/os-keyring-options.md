...
Every leaf result carries returned backend/device-binding generations. The registered wrapper verifies those values plus lifetime store instance and exact Arc before any blob/material/delete/missing receipt exits; the service cannot fill generations in after the call. Delete and fresh missing readback remain two operation-broker slots and two platform calls with a durable receipt checkpoint between them. Candidate discard/expiry specifically uses `CandidateDiscardConfirmationSlot::{RecordDelete,RecordMissingReadback}`; the latter maps to `Validate` and cannot be consumed until `CandidateDiscardDeleteCheckpoint{deleteDisposition,backendCompletedAt,deleteAppliedCas}` is durable. Activation runtime, activation durable failure and activationCleanup recovery keep the same three values in their distinct `ActivationOldRecordDeleteCheckpoint|ActivationOldRecordDurableCheckpoint|RecoveryOldRecordDeleteCheckpoint` roles; recovery reconstruction is checked and terminal `revokedAt` comes only from `backendCompletedAt`.
...
- authority/crash evidence proves candidate `RecordDelete → durable CandidateDiscardDeleteCheckpoint → Validate RecordMissingReadback → MissingReadbackVerified → terminal`, proves the three activation old-record role checkpoints retain identical disposition/time/CAS without unchecked conversion, and freezes the five staged resume fixtures `staged_resume_{intent,sources_scrubbed,cutover_committed,live_owner_minted,local_binding_finalized}_v1` with operation id, exact cumulative phase rows and CAS change;
