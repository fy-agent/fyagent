...
### D2 architecture rereview alignment (`ARR-001..003`)

- `ARR-001`: candidate discard/expiry has exact `CandidateDiscardConfirmationSlot::{RecordDelete,RecordMissingReadback}`, mapped respectively to `Delete` and `Validate`. Delete/already-missing must durably persist `CandidateDiscardDeleteCheckpoint {deleteDisposition,backendCompletedAt,deleteAppliedCas}`; only a fresh missing receipt consuming that CAS may advance `missingReadbackVerified` and then terminal immutable `discarded|expired`. No fifth recovery kind or ninth journal is added.
- `ARR-002`: normal activation and recovery respectively retain `ActivationOldRecordDeleteCheckpoint` and `RecoveryOldRecordDeleteCheckpoint`; `ActivationOldRecordDeleteApplied` carries the postcondition and `ActivationOldRecordDurableCheckpoint` is crash-visible. Each exact checkpoint preserves `{deleteDisposition,backendCompletedAt,deleteAppliedCas}`. The independent `Validate` missing receipt consumes the CAS; missing receipt and terminal supersession commit atomically with `revokedAt=backendCompletedAt`.
- `ARR-003`: staged resume digest preimage includes journal `operationId` and cumulative-field `StagedImportResumePhase::{Intent,SourcesScrubbed,CutoverCommitted,LiveOwnerMinted,LocalBindingFinalized}`: `intent` none; `sourcesScrubbed` adds `stagedSourceSetCasAfterScrub`; `cutoverCommitted` retains that CAS and adds `cutoverReceiptId`; `liveOwnerMinted` retains both and adds `promotedLiveOwner`; `localBindingFinalized` retains all three. Exact fixtures are `staged_resume_intent_v1|staged_resume_sources_scrubbed_v1|staged_resume_cutover_committed_v1|staged_resume_live_owner_minted_v1|staged_resume_local_binding_finalized_v1`; each fresh nonce/admission and phase/checkpoint transition increments revision and produces a new CAS.
- Slot accounting is `activation+recovery=10`, `candidate-discard=2`, total `12`; five delete→missing pairs contribute 10 delete/missing slots. This does not change the exact five hardware operations, 8 journals or 4 recovery kinds.
...
| ARR delete/missing checkpoints | #35/#41 authority boundary | #35 owns candidate and runtime/durable checkpoint algebra; #41 prepares activation/activation-recovery slots before its lease and cannot reconstruct disposition/time. Candidate, normal activation and recovery persist their exact named three-field checkpoint; fresh `Validate` missing consumes its CAS. Static assertions freeze `10+2=12` total slots and 10 delete/missing slots without changing five operations/8 journals/4 recovery kinds. |
| ARR staged resume CAS | Main-integration import boundary | Main integration owns journal operation-id binding, cumulative-field `StagedImportResumePhase`, promoted-owner/cutover receipts and the five named digest fixtures. Every nonce/admission/phase/checkpoint transition yields new CAS; #35/#55 cannot create a parallel preimage or reuse prior CAS. |
...
The source-freeze audit must compare every changed path against this budget and the §9.4 generated owner register. Every new existing-source path must have exactly one canonical owner; multiple behavioral rows never mean multiple writers. It also verifies core→owner adapters→main composition order; exact five-field resume result data with null/typed issue arms and envelope/audit separation; ARR-001 candidate two-slot/three-field/fresh-missing closure; ARR-002 named runtime/durable checkpoints and `revokedAt=backendCompletedAt`; ARR-003 operation-id plus exact cumulative five-phase preimage, five fixtures and every-transition CAS; `10+2=12`/10-delete-missing slot counts while retaining five operations/8 journals/4 recovery kinds; coverage receipt/factory `pub(crate)` with private fields; the sole named bridge constructor for unforgeable authority; atomic non-value-derived revision + fixed complete 11-domain identity + exact current expectations + adjacent observations; domain structural revision/presence/count; internal non-value-derived location IDs but no raw path/raw locator/value/value-derived digest; empty-proof rejection and fresh startup/summary/capture/Provider-delete bridge calls; 15+1 registration; durable/process identity separation; #35 registry/broker and both native Rust 1.85.0 records. Any new overlap, unregistered path, changed owner, or worker edit to a shared main-integration file requires re-review before implementation continues.
