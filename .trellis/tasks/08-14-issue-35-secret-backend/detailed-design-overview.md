...
re-read plan/candidate/record/backend/capability/owner expectations
  ...
  -> durable {deleteDisposition, backendCompletedAt, deleteAppliedCas} checkpoint
  -> independently authorized fresh missing readback
  -> atomic supersession tombstone with revokedAt=backendCompletedAt
  -> audit + terminal
```
...
Capture compensation, intentional-delete finalization and post-Provider-commit owner detach are also durable recovery kinds. Device-local state, recovery CAS preimage, journal pointer, impact/result and startup dispatch are the same four-arm tagged union; they do not collapse all failures into activation cleanup. Capture compensation and delete finalization each prepare delete and missing-readback as separate slots and persist a role-specific `{deleteDisposition,backendCompletedAt,deleteAppliedCas}` checkpoint between them; deleteFinalization can execute an admitted delete after an intent-only crash. Activation and activationCleanup retain that full checkpoint in every crash-visible recovery arm, so a later fresh missing receipt can atomically write supersession with `revokedAt=backendCompletedAt`. Exactly eight operation journals exist and there is no generic ninth recovery operation. Candidate explicit discard/expiry is not a fifth recovery kind, but its discard journal prepares two independent operation slots: candidate delete and `Validate` missing-readback. Delete fulfills a CAS reservation only by durably storing the same three-field checkpoint; the missing authority consumes it before terminalization. Until then the candidate remains `verifiedPendingPlan` with immutable `pendingTerminalDisposition=discarded|expired` and a reachable discard action; then and only then becomes terminal, drops the pending field and returns `refreshSummary` so any subsequent capture/rotation mints fresh authority.
