...
- [x] The cumulative branch diff is based on current `main` and contains no
      obsolete V1 renderer or historical task payload from PR #114.
- [x] Schema v19 upgrades transactionally to v20 with all three Change Plan
      tables/indexes; fresh databases start at the same shape.
- [x] Repeated create requests have unique plan/proof IDs and approval bindings;
      secret-bearing equality cannot be derived across plans.
- [x] Plan creation proves exactly one plan-ledger insert, zero Provider/live/
...
- [x] Digest mismatch, expiry, stale current/target/live baseline, and replay
      are rejected before the Provider writer and create no second job.
- [x] API-Key-only drift and process-private-proof loss reject an unapplied
      plan as stale with zero writer calls; a nonterminal job without its proof
      becomes recovery-required and is never replayed.
- [x] A valid apply invokes the existing Codex Provider switch once and only
      reports success when all required readback authorities match.
- [x] Writer failure, baseline restored, target reached, mixed/unknown state,
      and interrupted-job reconciliation are distinct and tested; reconcile
      performs no write replay.
- [x] The path-like display-name sanitizer behaves identically on macOS,
      Linux, and Windows path separators.
- [x] Focused Rust tests, schema migration tests, format, clippy, full Rust
      tests, repository contracts, `git diff --check`, and Trellis validation
      pass from the final commit.
- [x] The branch is pushed and a replacement PR referencing #55 and superseding
