...
本机固定版本来自 `research/session-recovery-20260921/local-cli-evidence.json`：`1.18.30`。同版本官方源码 tag 为 [`v1.18.30`](https://github.com/anomalyco/opencode/tree/v1.18.30)。本机已安装二进制是 `/Users/serendipity/.opencode/bin/opencode`；本报告不把当前 `dev` 源码混入版本结论。
...
版本证据：`research/session-recovery-20260921/local-cli-evidence.json` 记录 `v0.20.5`、upstream `f293e720`、本机携带补丁 `987064ca (+1 carried commit)`；源码树是 `/Users/serendipity/.hermes/hermes-agent`。
