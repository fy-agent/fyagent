...
同一 OpenCode 会话从 Windows 迁到 Mac：源 ID 相同，`source_path` 从 `sqlite:C:\Users\<username>` 变成 `sqlite:/Users/<username>`。用现有 `getSessionKey` 会当成两个会话，重复写入；若改用源 ID 直接当目标 ID，OpenCode「按 ID 写入、有冲突处理」（`恢复方案.md`）会打到已有行上。UX 的覆盖选项把这条变成用户可点的破坏操作：导入对话框打开期间目标机若已有新回复，覆盖会删掉那轮对话。
...
用户原文是 `请看 D:\work\app\bug.ts 第 12 行`。若导入时按「隔离绝对路径」改写成 `/Users/<username>/app/bug.ts`，目标模型看到的不是用户说过的话，下一轮引用会错。若 adapter 删除 assistant 的 `path` 字段而 OpenCode 1.18.30 需要它，导入会从「已证明」退回「未知」。
