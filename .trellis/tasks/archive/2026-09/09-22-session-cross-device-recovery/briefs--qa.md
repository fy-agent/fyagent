...
你是 Cursor 的测试架构师。交付 deliverables/test-plan.md 和 deliverables/test-cases.json。测试用例有ID、优先级、前置、合成fixture、步骤、精确预期、自动化层级和证据要求。重点：final-only保真、恶意/坏包、未知字段夹带、未完成轮次、重复/增量/另存、原生导入成功但本地receipt失败、版本不支持、目录不存在/跨OS路径、旧记忆文件不变、下一轮引用历史合成事实、重启、Mac到Windows与反向。每个provider分能力门槛，不虚报执行通过；提供可实际落地的测试顺序，不堆笛卡尔矩阵。只改本角色文件。