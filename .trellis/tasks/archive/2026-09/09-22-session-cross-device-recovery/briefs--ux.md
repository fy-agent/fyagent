...
基线仓库 /Users/<username>/fyagent，origin https://github.com/fy-agent/fyagent.git，分支 codex/frontend-interaction-v3-1-20260826，HEAD 60d699fa00b3f275dcda556afc74a21eaa05379d；工作区有大量已有界面 dirty changes，全部保护。读取 AGENTS.md 和 .trellis/workflow.md；本包仅方案/原型/隔离验证，禁止改生产源码、提交、发布、删除或写真实会话库。
...
你是 Antigravity UI/UX 设计师，模型 gemini-3.8-flash-high。读 ~/.codex/DESIGN.md 和现有 src/v2 的 shell、memory 页面与样式。交付 deliverables/ux-spec.md 和 deliverables/session-prototype.html（自包含可交互，不接真实服务）。需要列表/问答详情、筛选、导入导出、选择本地目录、版本不支持、最终答复不可判定、未完成轮次、重复导入、导入后待续聊验证等状态。产品界面自然中文，不暴露治理流程术语。保持现有品牌，用明确tokens，避免额外dashboard和层层向导。旧记忆入口安置给最小建议。HTML应可切换关键状态，禁止将假数据说成真实恢复。自检按钮/状态、键盘、窄窗。不得修改源码或其他角色文件。已有用户授权创建这些交付，直接完成。