...
复用 services/secret、database migration、现有 OAuth manager 协议逻辑和 Proxy resolver；引入 ManagedAuthRepository + SecretCredentialVault + RefreshCoordinator。SQLite 保存 opaque SecretHandle 与 metadata，SecretRef 保存版本化 token bundle。
