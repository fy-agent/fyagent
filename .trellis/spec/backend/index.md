...
| [Shurufa Companion Serial and Agent Bridge](./shurufa-companion.md)       | Native-owned Windows Board C HID auto-link pump, `shurufa_companion_*` commands, `VKEY_ASR/1` exactly-once admission, reuse of existing shurufa Agent/`enigo`, and ACL/handler registration.                                                                         |
...
| [External Agent P0 Safety](./external-agent-p0.md)                        | Catalog v4 (Grok Build + TRAE Work CN URL), runtime/launch authority, closed macOS bundle-id / Windows PE ProductName observation, Skills including disk observation, Qoder Hooks, TRAE GET observation/`state.vscdb`, OpenCode `opencode.json` persist, Agent install/action façade, narrow permissions, and secret boundaries.   |
...
| [Windows Runtime Security](./windows-runtime-security.md)                 | Frozen Explorer-user identity/paths, Alice-owned Tauri state, validated foreground external links and closed desktop-agent `.exe` launch through Explorer, untrusted single-instance input, elevated CLI boundary, and Agent Catalog CLI/auth fail-closed on formal elevated Windows.                                                          |
...
| [Repository Task Runner](./task-runner-contract.md)                       | Canonical mise metadata, argv transport, DAG effects, interactive+raw console ownership and host process-tree teardown, generic direct-session prearchive verification, maintenance safety, and generated task documentation.                                                                              |
