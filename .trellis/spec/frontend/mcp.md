# Renderer MCP Management UI Contract

## 1. Scope / Trigger

Read this contract before changing the Renderer MCP installed/discovery views,
...
- `src/pages/mcp/Page.tsx`, `Discovery.tsx`, `InstallDialog.tsx`, and
  `catalog.ts` for current route behavior;
- `src/shared/features/mcp.ts` for renderer MCP types;
- `src/shared/features/mcpSecurity.ts` and the MCP helpers in
  `src/shared/features/helpers.ts` for display/search redaction and editor
  serialization;
- `src/shared/features/ports.ts` and
  `src/shared/platform/tauri/feature-ports/simple.ts` for `McpPort` IPC;
- `src/shared/features/queries.ts` for the installed-server query.

Shared target presentation is owned by
[Renderer Shared Assignment](./assignments.md). Native validation, persistence,
...
- `tests/renderer/features/mcpSecurity.test.ts` and
  `tests/renderer/features/mcpCatalog.test.ts`: URL/argument redaction, search tokens,
  recipe identity, and reviewed catalog projection;
- `tests/renderer/shared/AssignmentPanel.test.tsx`: semantic seven-target controls;
- `tests/renderer/app/architecture.test.ts`: route/shared component ownership and no
...
const url = server.server.url ? redactMcpUrl(server.server.url) : undefined;
