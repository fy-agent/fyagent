...
| [V2 Skills and MCP Feature Contract](./v2-skills-mcp.md)                   | Seven catalog-ordered V2 Skill and MCP targets, shared `AssignmentPanel` for install radio and assignment switches, disk-observed installed Skills, Skill 市场 discovery (`searchSkillHub` via official `/api/skills` page/category, 21 per page, no GitHub 管理仓库 / skills.sh tab), Skills page-level discovery scroll, leftover Gemini/Hermes round-trip, Qoder/TRAE/WorkBuddy live MCP files, secret handling, and responsive UI. |
