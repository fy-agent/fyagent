...
2. Read the nearest relevant guideline and inspect the existing feature,
   primitive, and executable tests. Any user-visible text or public product
   documentation change must also follow
...
| [V2 Agent and Models Contract](./v2-agent-models.md)                       | Catalog v4 seven-entry order, shared Agent/Models chrome, closed Agent install/action port, WorkBuddy save and Codex Quick Setup through Change Plan, OpenCode persistence, direct targeted Claude/Grok Quick Setup writes with native path/rolling-backup disclosure, revision-based `待保存` and stale-probe invalidation, protocol-correct model probes except Qoder/TRAE, shared SecretInput geometry, vendor icons, and secrets. |
| [V2 Skills and MCP Feature Contract](./v2-skills-mcp.md)                   | Seven catalog-ordered V2 Skill and MCP targets, shared `AssignmentPanel` for install radio and assignment switches, Installed/Discover tabs on the same header row as page actions, disk-observed installed Skills, Skill 市场 discovery (`searchSkillHub` via official `/api/skills` page/category, 21 per page, no GitHub 管理仓库 / skills.sh tab), Skills page-level discovery scroll, leftover Gemini/Hermes round-trip, Qoder/TRAE/WorkBuddy live MCP files, secret handling, and responsive UI. |
...
| [External Agent P0 Safety](../backend/external-agent-p0.md)                | Cross-layer QoderWork/TRAE Work native command, persistence, network, permission, Agent install/action façade, and evidence boundary.                                                                                                                                                                                                                                                                                                                                                                                               |
...
| [User-Facing Copy](./user-facing-copy.md)                                  | Outcome-first interface and public-documentation language, prohibited implementation narration, error/empty-state structure, terminology, translation parity, and copy regression checks.                                                                                                                                                                                                                                                                                                                |
...
| [Codex Provider Configuration](../backend/codex-provider-configuration.md) | Codex native-capability controls, warnings, live-change result, trusted restart handoff, managed ChatGPT `credential_id` vs workspace routing, and file-only native `auth.json` projection.                                                                                                                                                                                                                                                                                                                                           |
