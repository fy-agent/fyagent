...
| [V2 Agent and Models Contract](./v2-agent-models.md)                       | Catalog v4 seven-entry order (Grok Build after WorkBuddy), shared `PRODUCT_DIRECTORY` / `CatalogOfficialLinks`, Agent detail only `direct` capabilities plus page-local 「产品介绍」, Qoder unsupported third-party models (MCP jump), TRAE GET observation, OpenCode native persist, Codex installer without package-hash admission, WorkBuddy, Provider quick setup including Grok Build and Claude `/v1` warn, existing-Codex-Provider Change Plan preview/apply observation, post-fetch model probe except Qoder/TRAE, vendor icons, and secrets. |
