# Renderer Navigation and Persistent Route Contract

## 1. Scope / Trigger

Read this contract before changing Renderer primary-route registration, sidebar
...
- `src/app/router.tsx` and `src/app/primaryPages.tsx` for routing and
  literal page loaders;
- `src/app/PersistentPrimaryOutlet.tsx`,
  `src/shared/ui/PersistentSurface.tsx`, and
  `src/shared/ui/usePersistentSearchParams.ts` for visited-page lifetime and
  route-owned query state;
- `src/widgets/app-shell/SideNavigation.tsx` for visible navigation;
- `src/shared/features/agent-navigation.ts` for Agent return semantics;
- `src/shared/ui/PrimaryBlocker.tsx` for one shell-level leave blocker.

Visual material, native title-bar behavior, shared selection-lens geometry and
motion are owned by [Renderer Window Shell and Interaction](./window-shell.md).
...
- `tests/renderer/app/router-shell.test.tsx` covers route/active-link behavior,
  persistence, lazy/prefetch boundaries, redirects, and sidebar integration.
- `tests/renderer/widgets/app-shell/SideNavigation.test.tsx` covers grouping,
  collapsed availability, active semantics, Agent return preservation, and
  Arrow/Home/End focus behavior.
- `tests/renderer/shared/usePersistentSearchParams.test.tsx` covers visible snapshots,
  hidden reads, hidden setter suppression, reactivation, and sticky values.
- `tests/renderer/features/agentNavigation.test.ts` covers exact accepted tuples,
...
- Feature tests that use `usePrimaryBlocker` cover blocked transition,
  ...
  provider.
- `tests/renderer/app/architecture.test.ts` asserts the closed route registry,
  literal imports, Renderer boundaries, and the development-only UI Lab rule.
- Browser coverage in `tests/browser/shell.spec.ts` and affected feature
