# Renderer Shared Assignment Contract

## 1. Scope / Trigger

Read this contract before changing the Renderer target order, assignment DTO
...
- `src/shared/features/directory.ts` for the closed seven-target IDs/order;
- `src/shared/features/assignments.ts` for Skill/MCP assignment projection;
...
Feature contracts remain in [Renderer Skills](./skills.md) and
[Renderer MCP](./mcp.md). Native ordering and cross-resource failure semantics
remain in [Skill Management](../backend/skill-management.md) and
[MCP Management](../backend/mcp-management.md).

## 2. Signatures

The Renderer presentation target set and order are closed:
...
`claude-desktop=false` and `openclaw=false`; the extra fields are not Renderer
rendered targets.
...
- The target registry in `directory.ts` owns Renderer order and labels. Feature pages
  do not maintain independent arrays or reorder targets from observed data.
- The target unions/tuples provide a compile-time closed set for normal Renderer
...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Typed Renderer code needs a target outside the seven-ID set      | Reject the design; update the shared registry/contract deliberately rather than adding an ad hoc row.                                                                    |
...
- **Base:** a compatibility Skill snapshot contains `openclaw`; Renderer keeps it out
...
- `tests/renderer/features/authoritativeAssignment.test.tsx`: one pending mutation,
