...
- Only the directory branch may show onboarding. A valid explicit `target`
  remains authoritative. Query activity follows persistent route visibility.
...
Wrong: `if (!localStorage.getItem("welcome")) showGuide()` or hiding unsupported
actions by rewriting their runtime status. Correct: native first-use state owns
eligibility; only catalog summary prose is positive-only, while actual operation
errors, unknown states and security confirmations stay visible.
