# Renderer Window Shell and Interaction Contract

## 1. Scope / Trigger

Read this contract before changing Renderer shell composition, native-overlay chrome,
top-bar drag regions, frontend-readiness signalling, shared selection material,
collapse animation, motion ownership, external-link interaction, or Renderer import
boundaries.

Primary owners are:

- `src/widgets/app-shell/AppShell.tsx`, `TopBar.tsx`, and
  `ContentViewport.tsx` for shell composition and Renderer chrome;
- `src/shared/platform/runtime.ts` and the platform Port surface for runtime
  detection, frontend readiness, window/native effects, and external opening;
- `src/shared/ui/SelectionLens.tsx`, `selection-lens.css`,
  `GlassMaterial.tsx`, `motion.ts`, and `Collapsible.tsx` for shared
  interaction material and motion;
- `src/shared/features/controls/ExternalLinkButton.tsx` and
  `src/shared/features/provider.tsx` for HTTP(S) opening and shared feedback;
- `tests/renderer/app/architecture.test.ts` for enforceable Renderer dependency and
  ownership boundaries.

Route registration, persistent page lifetime, sidebar navigation and return
queries are owned by
[Renderer Navigation and Persistent Route](./navigation.md). Native window
...
- Renderer does not draw custom minimize/maximize/close controls. Host/Tauri window
...
### External links and Renderer architecture
...
- The shared opener permits one in-flight external open at a time, exposes the
  ...
  native requests.
- Production imports follow role boundaries under `src`. Portable Codex
  contracts live in `@/domain/codex-desktop`; installer UI stays in
  `shared/codex-desktop`. There is no second/legacy renderer or facade bridge.
- Direct `@tauri-apps/*` imports are limited to
  `src/shared/platform/tauri/`. Dependency direction remains root → app;
  app → pages/widgets/shared/dev; pages → same-page/shared/domain; widgets →
  widgets/shared/domain; shared → shared/domain; domain → domain.
...
| Renderer imports legacy/root UI, direct Tauri outside platform/tauri, or a prohibited cross-layer module | Architecture check fails.                                                                                       |
...
- `tests/renderer/widgets/app-shell/TopBar.test.tsx` covers platform-gated overlay
  markup, traffic-light reserve, and the sole drag region.
- `tests/renderer/app/router-shell.test.tsx` covers shell composition and the absence
