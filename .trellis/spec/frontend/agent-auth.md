# Renderer External Agent Auth UI Contract
...
Renderer Agents page.

Primary owners are:

- `src/shared/features/agent-auth.ts` for closed DTOs, strict parsers, and
  `AgentAuthPort`;
- `src/shared/platform/tauri/feature-ports/agentAuth.ts` for Tauri IPC
  adaptation;
...
[Renderer Agent Directory](./agent-directory.md) and
[Renderer Navigation](./navigation.md).

## 2. Signatures

`AgentAuthPort` is the only Renderer access to the native Auth surface:
...
- Account/connection/request-source semantics on the destination are owned by
  [Renderer Managed Accounts and Authentication](./managed-auth.md).
