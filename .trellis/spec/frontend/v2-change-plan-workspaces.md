# V2 Change Plan Workspace Contract

## 1. Scope / Trigger

Read this contract before changing V2 Change Plan preview/apply presentation,
...
- `src/v2/shared/features/change-plans-ui/ApplyWorkspace.tsx`,
...
- `src/v2/pages/models/apply/SavePlanWorkspace.tsx` plus the Codex/WorkBuddy
  wrappers for Models-specific save admission;
- `src/v2/pages/auth/CodexRequestSource.tsx` for the sole saved Codex
  Provider-switch entry;
- `src/v2/shared/features/change-plans.ts` and `queries.ts` for strict DTO/Port
...
[V2 Models](./v2-models.md); account/connection ownership remains in
[V2 Managed Accounts](./v2-managed-auth.md).
...
- `tests/v2/pages/models/apply/architecture.test.ts`: one save controller,
  Query-owned automatic reads, Auth-only source switching, and no local timer
  or Mutation-cache orchestration;
- `tests/v2/pages/models/apply/ApplyWorkspace.test.tsx`,
  ...
  adapters;
- `tests/v2/pages/auth/CodexRequestSource.test.tsx`: one apply, both readbacks,
  failure/retry without rewrite, unknown admission, hidden reads, and closed
  return context;
- `tests/v2/pages/models/Page.test.tsx` and
  `tests/v2/pages/auth/Page.test.tsx`: owner placement, reverse invalidation,
