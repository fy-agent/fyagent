# Renderer Model Configuration Contract

## 1. Scope / Trigger

Read this contract before changing the Renderer Models route, target selection,
...
- `src/pages/models/QoderModelsPanel.tsx` and `TraeModelsPanel.tsx` for the
  unsupported/read-only targets;
- `src/pages/models/quickSetup.ts` and `workBuddyModels.ts` for validation
  and draft shaping;
- `src/pages/models/apply/**` for Models-specific Codex/WorkBuddy save
  adapters over the shared Change Plan workspace;
- `src/shared/features/models.ts`, `change-plans.ts`, and `ports.ts` for the
  DTOs and five actual Port owners;
- `src/shared/platform/tauri/feature-ports/models.ts`, `changePlans.ts`, and
...
[Managed Accounts](./managed-auth.md#saved-codex-request-source).
...
[Renderer Change Plan Workspaces](./change-plan-workspaces.md). Keep this heading
...
Run the focused Renderer checks through the repository task runner. Required
assertion owners include:

- `tests/renderer/pages/models/Page.test.tsx`: exact target order/default,
...
- `tests/renderer/pages/models/quickSetup.test.ts`: closed target parsing, URL/key
...
- `tests/renderer/pages/models/apply/*.test.tsx`: neutral preview, one apply under
...
- `tests/renderer/app/router-shell.test.tsx`: persistent Models lifetime and hidden
