...
- **macOS Developer ID and one notarization:** GitHub Actions signs the
  ...
  then notarizes only the signed DMG. After Apple accepts that submission, the
  workflow staples the DMG and the app from the same ticket. The ZIP contains
  that stapled app. This avoids two serial Apple notarization waits.
...
new annotated tag on the exact `main` commit whose push `CI / Required` result
succeeded. A failed Release workflow does not move that tag; a later source
fix requires a new stable version.
