...
serial app-then-DMG notarization wait and did not publish. FyAgent v0.4.2 is an
annotated tag on the exact `main` commit whose push `CI / Required` result
succeeded. If that formal run fails before a GitHub Release exists, operators
may force-update unpublished `v0.4.2` onto a later exact-source SHA instead of
bumping the Cargo version. The workflow itself never moves or deletes the tag.
