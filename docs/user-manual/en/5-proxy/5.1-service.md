...
sequenceDiagram
    ...
    participant Proxy as Local Proxy (FyAgent)
...
After starting the proxy and enabling app takeover, FyAgent modifies app configurations:
...
The API format is configured per-provider in the [Advanced Options](../3-providers/3.1-add.md#api-format-claude-only) when adding or editing a Claude provider.
...
When stopping the proxy, FyAgent will:
