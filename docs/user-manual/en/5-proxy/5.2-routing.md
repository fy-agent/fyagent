...
App routing means letting FyAgent route a specific application's API requests through the local routing service.
...
When routing is enabled, FyAgent modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, FyAgent will:
...
Before enabling routing, FyAgent backs up the original configuration and restores it when disabled.
