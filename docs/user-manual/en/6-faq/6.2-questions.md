...
The complete app is signed with an Apple Developer ID and notarized by Apple,
and the DMG container is signed and stapled. Download only from the
[Releases page](https://github.com/fy-agent/fyagent/releases), verify the
exact build's evidence, and follow the
[installation guide](../1-getting-started/1.2-installation.md). If Gatekeeper
still blocks the first launch, use the safe
[Open Anyway procedure](../1-getting-started/1.2-installation.md#if-gatekeeper-blocks-the-first-launch).
...
Only **official subscription types** (Claude / Codex / Gemini official login, GitHub Copilot, Codex OAuth reverse proxy) automatically display the quota after enabling the provider. **All other providers** (including Token Plan and third-party balance queries) need the **Usage Query** switch to be manually turned on and a built-in template selected in the provider card — because the same request URL may expose both "plan" and "balance" query modes, requiring you to pick the right one. See [3.5 Usage Query → Manual Enable](../3-providers/3.5-usage-query.md#manual-enable-built-in-templates--custom-scripts).
...
See the complete Device Code login flow (verification code + browser authorization), both entry points (Add Provider panel / OAuth Auth Center), multi-account management, and common failure scenarios in [3.1 Add Provider → Codex OAuth Reverse Proxy (Claude Provider)](../3-providers/3.1-add.md#codex-oauth-reverse-proxy-claude-provider).
...
See the current full disclaimer in [3.1 Add Provider → Codex OAuth Reverse Proxy](../3-providers/3.1-add.md#codex-oauth-reverse-proxy-claude-provider).
...
1. Visit [GitHub Issues](https://github.com/fy-agent/fyagent/issues)
