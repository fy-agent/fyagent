# 6.3 Deep Link Protocol

## Overview

FyAgent retains native parsing and safety validation for the `fyagent://` protocol. This page is a protocol compatibility reference; the current interface does not yet connect a general deep-link import confirmation flow. Opening a link does not establish that an import succeeded. Use the existing Models, MCP and Prompts management pages for configuration.
...
## Link and Validation Boundaries

The project no longer distributes an offline HTML generator or asks users to download a test page. Automated tests cover parsing, invalid inputs and no-side-effect boundaries. A link is input data, not user authorization. Never put API keys, access tokens or private configuration in public links.
