...
On first launch, FyAgent migrates legacy primary and provider configuration
JSON into the `fyagent.db` SQLite database. This does not replace every JSON
contract: `settings.json` and WorkBuddy's `models.json`, among others, remain
current configuration files.

- Automatic migration of eligible legacy configuration on first launch
