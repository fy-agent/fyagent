# 6.4 Environment Variable Conflicts

## Overview

FyAgent automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with FyAgent configuration
[Expand] [Dismiss]
```
...
| Shell Configuration | macOS shell configuration file |
...
3. FyAgent will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `~/.fyagent/env-backups/`
...
If you prefer not to use FyAgent to remove variables, you can handle them manually:
...
- FyAgent provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use FyAgent to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file: `~/.fyagent/env-backups/`
