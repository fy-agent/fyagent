...
The following three categories automatically display the quota at the bottom of the provider card after the provider is enabled — no additional configuration required:
...
FyAgent provides **ready-to-use built-in templates** for the following categories — no script writing required:
