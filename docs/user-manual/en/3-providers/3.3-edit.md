...
FyAgent provides rich icon customization features:
...
This ensures FyAgent and CLI tool configurations stay in sync.
...
See [3.1 Add Provider — Auto-Fetch Models](./3.1-add.md#auto-fetch-models) for full details.

## Common Config Toggles (Claude)

When editing a Claude provider, quick toggle switches are available above the JSON editor for common settings like Tool Search, Disable Auto Upgrade, Teammates, and High Effort. See [3.1 Add Provider — Claude Common Config Toggles](./3.1-add.md#claude-common-config-toggles) for details.
...
When adding a provider, FyAgent automatically extracts endpoint URLs from the configuration.
