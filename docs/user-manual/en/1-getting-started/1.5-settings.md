...
Click **Refresh** to re-detect. When Grok Build is missing, use **Install**. When a newer Grok Build is available, use **Update**. Installs and updates run in the background; the button shows progress and the version refreshes when the readback succeeds.
