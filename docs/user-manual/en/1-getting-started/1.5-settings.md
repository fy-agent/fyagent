...
FyAgent includes **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
Start/stop the local proxy service, configure the listen address and port. See [5.1 Proxy Service](../5-proxy/5.1-service.md) for details.

### Failover

Configure failover queues and automatic switching strategies by app (Claude/Codex/Gemini). See [5.3 Failover](../5-proxy/5.3-failover.md) for details.
...
The **OAuth Auth Center** (Beta) provides unified management for third-party OAuth credentials. It supports three account types:
...
Automatically detects installed CLI tool versions. **Install and update in Settings are available only for Grok Build.** Claude Desktop, Codex Desktop, and OpenCode Desktop are managed from the Agent page, not by pasting npm commands here.

| Tool | Detection | Settings lifecycle |
| -------- | ------------------------------- | ------------------ |
| Grok Build | Current version, latest version | Install and update |
| Claude | Current version, latest version | Read-only |
| Codex | Current version, latest version | Read-only |
| Gemini | Current version, latest version | Read-only |
| OpenCode | Current version, latest version | Read-only |
| OpenClaw | Current version, latest version | Read-only |
| Hermes | Current version, latest version | Read-only |

Click **Refresh** to re-detect. When Grok Build is missing, use **Install** (this installs the official npm package through mainland npm mirrors). You can also choose the official CLI installer explicitly. When a newer Grok Build is available, use **Update**. A successful install only means the CLI is on this computer; sign-in and inference still depend on your network. Installs and updates run in the background; the button shows progress and the version refreshes when the readback succeeds.
