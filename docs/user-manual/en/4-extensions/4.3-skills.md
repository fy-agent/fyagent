...
For a GitHub install, FyAgent waits for at most 60 seconds. It first places the complete skill in the single source of truth (SSOT), `~/.fyagent/skills/` by default. It then saves the database record and content hash before synchronizing the skill to each enabled application. Updates, disable operations, and uninstalls therefore share one managed source.

If you switch source storage to `~/.agents/skills/`, the application directories below stay the same; only the SSOT location changes.
...
Synchronization can use Auto, Symlink, or Copy:

- **Auto** tries a symbolic link first and falls back to a copy. If the destination is already a regular directory, it keeps using a copy instead of changing its type unexpectedly.
- **Symlink** requires a symbolic link and reports an error if it cannot create one.
- **Copy** always copies the files and is useful where symbolic links are unavailable or inconvenient.

Skills are therefore not “always linked” or “always copied.” The final behavior depends on the setting, platform permissions, and the current destination.
...
- Removes the skill from all app directories (Claude, Codex, Gemini, Grok Build, OpenCode, Hermes)
...
FyAgent retains the 20 most recent uninstall backups. When the limit is exceeded, it removes the oldest entries first. Keep important skills in your own version-control or backup system as well.
...
Skills support **automatic update detection** and **batch updates** — no more uninstall-and-reinstall.
...
The **source storage location** for skills can be switched between two locations:
...
FyAgent integrates **skills.sh** public registry search so you can discover community skills directly inside the app.
...
FyAgent handles broken links and empty descriptions from skills.sh so community skill metadata is displayed more reliably.

## Installation safety boundary

Installation rejects unsafe directory names, paths that escape the repository or SSOT, invalid archive content, and same-directory-name conflicts from different repositories. Extraction and synchronization also validate symbolic links and resolved paths so that a skill cannot write outside the intended target through a crafted path.

These checks constrain where files land; they do not prove that a community skill's prompts or scripts are trustworthy. Read its `SKILL.md` and bundled scripts before enabling it, including the tools, network services, and local files it can access. Do not install an unknown skill directly into a working environment you care about.
