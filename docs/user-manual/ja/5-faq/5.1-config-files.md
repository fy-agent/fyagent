...
初回起動時に、FyAgent は旧形式の主要設定とプロバイダー設定の JSON を SQLite
データベース `fyagent.db` に移行します。すべての JSON 設定契約を置き換えるわけ
ではなく、`settings.json` や WorkBuddy の `models.json` などは引き続き現在の
設定ファイルです。

- 初回起動時に対象となる旧設定を自動的に移行
- 移行成功後に通知を表示
- 旧設定ファイルはバックアップとして保持
