...
> **注意**：OpenClaw と Claude Desktop は現在 FyAgent MCP 同期に対応していません。MCP 機能は Claude、Codex、Gemini、OpenCode、Hermes に対応しています。

### スイッチの動作

あるアプリのスイッチをオンにすると、FyAgent は以下を実行します：
...
あるアプリのスイッチをオフにすると、FyAgent は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、FyAgent にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode/Hermes）
3. FyAgent が既存の設定を読み取ってインポート
