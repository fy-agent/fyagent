...
完整操作请参阅 [3.6 Claude Desktop](./3.6-claude-desktop.md)。
...
| 智谱 GLM en       | 智谱 AI（英文版）       |
| 百炼              | 阿里云百炼              |
| Kimi k3.5         | Moonshot Kimi-k3.5 模型 |
| Kimi For Coding   | Kimi 编程专用模型       |
| StepFun           | 阶跃星辰 Step模型       |
| ModelScope        | 魔搭社区                |
| KAT-Coder         | KAT-Coder 模型          |
...
| `model`                    | 是   | 使用的模型（如 `gpt-6.2`、`gpt-4o`）                                                              |
...
`fyagent://` 保留为原生协议兼容能力；当前界面尚未接入通用导入确认流程。请通过模型管理页面添加配置，不要将打开链接视为导入成功。项目不再提供离线 HTML 生成工具；协议和安全边界见[深度链接协议](../6-faq/6.3-deeplink.md)。
...
> **注意**：API 格式转换由代理服务处理。使用非 Anthropic 格式时，需要开启代理并启用应用接管才能正确转换请求/响应。详见 [5.1 代理服务](../5-proxy/5.1-service.md)。
...
- **前提条件**：必须开启[本地路由服务](../5-proxy/5.1-service.md)并启用 Codex 接管，转换才会生效；使用过程中需保持本地路由开启
...
| 能力               | 含义                                     | 代表供应商                                                |
| ------------------ | ---------------------------------------- | --------------------------------------------------------- |
| **支持思考等级**   | 可调节思考强度（low / medium / high 等） | DeepSeek、OpenRouter、StepFun（仅 `step-4.5-flash-2603`） |
