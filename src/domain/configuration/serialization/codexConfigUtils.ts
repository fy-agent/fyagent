...
import type { CodexApiFormat } from "@/domain/configuration/types";
import {
  isPlainObject,
  isSubset,
  sanitizeSnippet,
} from "@/domain/configuration/serialization/providerConfigStructural";
import { normalizeTomlText } from "@/domain/configuration/serialization/textNormalization";
...
// JSON's string encoder supplies the shared quote, slash and C0 escapes used
// by TOML basic strings; do not maintain a second character-escaping table.
const tomlBasicString = (value: string): string => JSON.stringify(value);
...
export const isCodexGoalModeEnabled = (
  ...
  try {
    ...
    try {
      ...
      return isPlainObject(parsed.features) && parsed.features.goals === true;
    } catch {
...
export const isCodexRemoteCompactionEnabled = (
  ...
  try {
    ...
    try {
      ...
      const provider = isPlainObject(providers)
        ? providers[providerId]
        : undefined;
      return isPlainObject(provider) && provider.name === "OpenAI";
    } catch {
      // Fall back to line scanning while the user is editing invalid TOML.
    }
...
export const updateCodexExperimentalBearerToken = (
  ...
  if (!trimmed) {
    ...
    const encoded = tomlBasicString(trimmed);
    ...
    lines[tokenLineIndex] = TOML_EXPERIMENTAL_BEARER_TOKEN_REPLACE_PATTERN.test(
      ...
      : `experimental_bearer_token = ${encoded}`;
  }
  return finalizeTomlText(lines);
};
