...
import { isPlainObject } from "./providerConfigStructural";
...
export const validateToml = (text: string): string => {
  ...
  try {
    ...
    return e instanceof Error && e.message ? e.message : "parseError";
  }
};

/**
 * 将 McpServerSpec 对象转换为 TOML 字符串
 * 使用 smol-toml 的 stringify，自动处理转义与嵌套表
 * 保留所有字段（包括扩展字段如 timeout_ms）
 */
export const mcpServerToToml = (server: McpServerSpec): string => {
  // 先复制所有字段（保留扩展字段）
  const obj: Record<string, unknown> = { ...server };
...
export const tomlToMcpServer = (tomlText: string): McpServerSpec => {
  ...
  if (isPlainObject(parsed.mcp_servers)) {
    const serverIds = Object.keys(parsed.mcp_servers);
    if (serverIds.length > 0) {
      const firstServer = parsed.mcp_servers[serverIds[0]];
  ...
  if (isPlainObject(parsed.mcp)) {
    const mcpObj = parsed.mcp;
    if (isPlainObject(mcpObj.servers)) {
 ...
 * 保留所有字段（包括扩展字段如 timeout_ms）
 */
function normalizeServerConfig(config: unknown): McpServerSpec {
  ...
  if (type === "stdio") {
    ...
    if (config.args && Array.isArray(config.args)) {
      server.args = config.args.map((arg: unknown) => String(arg));
