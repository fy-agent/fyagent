...
describe("Grok Build config", () => {
  it("builds the expected provider TOML", () => {
    ...
    expect(parsed).toHaveProperty(["models", "default"], "grok-4.5");
    expect(parsed).toHaveProperty(["model", "grok-4.5"], {
      model: "grok-4.5",
...
`;
    ...
    expect(parsed).toHaveProperty(
      ["model", "env-profile", "env_key"],
      "XAI_API_KEY",
    );
    expect(parsed).not.toHaveProperty(["model", "env-profile", "api_key"]);
  });
  ...
  it("renames the selected profile without leaving the old table behind", () => {
    ...
    const parsed = parseToml(renamed);

    expect(parsed).toHaveProperty(["models", "default"], "new-profile");
    expect(parsed).toHaveProperty(
