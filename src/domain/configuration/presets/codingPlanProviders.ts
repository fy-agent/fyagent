 ...
 * 新建自动注入 + 托盘识别全部复用）。
 */
import { createUsageScript } from "@/domain/configuration/types";
import { TEMPLATE_TYPES } from "@/domain/configuration/presets/constants";
import { isPlainObject } from "../serialization/providerConfigStructural";
...
>(appId: string, provider: T): T {
  ...
  const baseUrl = isPlainObject(env) ? env.ANTHROPIC_BASE_URL : undefined;
