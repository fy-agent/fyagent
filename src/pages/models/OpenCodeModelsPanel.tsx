...
export function OpenCodeModelsPanel({
  ...
  const providers = (snapshotQuery.data?.providers ?? []).filter(
  ...
  const saveRequest = async (
    ...
    if (readOnlyProvider || writeLock.current || apiWritesBlocked) return;
    writeLock.current = true;
    setBusy("save");
    clear();
  ...
  const filteredExistingIds = filterModelIds(modelIds, existingSearch);
